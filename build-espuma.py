"""
Construye la categoria Espuma: sub-catalogo + 3 paginas de pedido.
- arte-mural-espuma.html
- pedido-espuma-vertical.html
- pedido-espuma-apaisado.html
- pedido-espuma-cuadrado.html
"""
import os
import sys
...
# Imagen ya generada (foam board real look)
IMG_BASE = "https://d8j0ntlcm91z4.cloudfront.net/user_3DYM1J3h5iy8QkCLK3SvRJRjyvr/hf_20260524_071634_c287ecec-1d21-43c7-b2d4-51df8779c72b.png"
IMG_HOVER = "https://d8j0ntlcm91z4.cloudfront.net/user_3DYM1J3h5iy8QkCLK3SvRJRjyvr/hf_20260524_071641_8d988ede-fdae-4152-80e8-5400ff066d57.png"

# ===== TAMAÑOS =====
# Vertical (10 tamaños) — orientacion vertical
SIZES_VERTICAL = [
    ("20x30 cm / 8x12\"",   "20 × 30 cm  ·  8 × 12\""),
    ("25x35 cm / 10x14\"",  "25 × 35 cm  ·  10 × 14\""),
    ("30x40 cm / 12x16\"",  "30 × 40 cm  ·  12 × 16\""),
    ("30x45 cm / 12x18\"",  "30 × 45 cm  ·  12 × 18\""),
    ("40x50 cm / 16x20\"",  "40 × 50 cm  ·  16 × 20\""),
    ("40x60 cm / 16x24\"",  "40 × 60 cm  ·  16 × 24\""),
    ("50x70 cm / 20x28\"",  "50 × 70 cm  ·  20 × 28\""),
    ("60x80 cm / 24x32\"",  "60 × 80 cm  ·  24 × 32\""),
    ("60x90 cm / 24x36\"",  "60 × 90 cm  ·  24 × 36\""),
    ("70x100 cm / 28x40\"", "70 × 100 cm  ·  28 × 40\""),
]

# Apaisado (10 tamaños) — dimensiones invertidas
SIZES_APAISADO = [
    ("30x20 cm / 12x8\"",   "30 × 20 cm  ·  12 × 8\""),
    ("35x25 cm / 14x10\"",  "35 × 25 cm  ·  14 × 10\""),
    ("40x30 cm / 16x12\"",  "40 × 30 cm  ·  16 × 12\""),
    ("45x30 cm / 18x12\"",  "45 × 30 cm  ·  18 × 12\""),
    ("50x40 cm / 20x16\"",  "50 × 40 cm  ·  20 × 16\""),
    ("60x40 cm / 24x16\"",  "60 × 40 cm  ·  24 × 16\""),
    ("70x50 cm / 28x20\"",  "70 × 50 cm  ·  28 × 20\""),
    ("80x60 cm / 32x24\"",  "80 × 60 cm  ·  32 × 24\""),
    ("90x60 cm / 36x24\"",  "90 × 60 cm  ·  36 × 24\""),
    ("100x70 cm / 40x28\"", "100 × 70 cm  ·  40 × 28\""),
]

# Cuadrado (5 tamaños, del screenshot de Angel)
SIZES_CUADRADO = [
    ("20x20 cm / 8x8\"",   "20 × 20 cm  ·  8 × 8\"  (cuadrado)"),
    ("30x30 cm / 12x12\"", "30 × 30 cm  ·  12 × 12\"  (cuadrado)"),
    ("40x40 cm / 16x16\"", "40 × 40 cm  ·  16 × 16\"  (cuadrado)"),
    ("50x50 cm / 20x20\"", "50 × 50 cm  ·  20 × 20\"  (cuadrado)"),
    ("60x60 cm / 24x24\"", "60 × 60 cm  ·  24 × 24\"  (cuadrado)"),
]
...
def price(cost_min, cost_max, ship_min, ship_max, pos, total):
    cost = interp(cost_min, cost_max, pos, total)
    ship = interp(ship_min, ship_max, pos, total)
    return round((cost + ship) * 2)

# Vertical/Apaisado: cost $12.12 - $41.33, envio $11.15 - $19.02
prices_vert = [price(12.12, 41.33, 11.15, 19.02, i, 10) for i in range(10)]
prices_apai = prices_vert  # mismo precio
# Cuadrado: cost $11.01 - $27.55, envio $11.15 - $19.02
prices_cuad = [price(11.01, 27.55, 11.15, 19.02, i, 5) for i in range(5)]

print("Vertical: ", prices_vert)
print("Apaisado: ", prices_apai)
print("Cuadrado: ", prices_cuad)
...
<title>Impresión en espuma · Kodara Print Studio</title>
...
<header class="page-header">
  <div class="page-header-inner">
    <div class="page-tag">Arte Mural · Impresión en espuma</div>
    <h1 class="page-title">
      Personaliza tu <span class="grad">impresión en espuma</span>.
    </h1>
    <p class="page-sub">
      Panel ligero, resistente y económico. Disponible en formato vertical, apaisado o cuadrado. Listo para colgar.
    </p>
  </div>
</header>
  ...
  <div class="sub-services">
    ...
    <article class="sub-card">
      <div class="sub-card-img">
        <span class="sub-card-price">Desde ${prices_vert[0]}</span>
        <img class="img-base" src="{IMG_BASE}" alt="Panel vertical en espuma">
        <img class="img-hover" src="{IMG_HOVER}" alt="Panel vertical en espuma en pared">
      </div>
      <div class="sub-card-body">
        <h2 class="sub-card-title">Panel vertical en espuma</h2>
        <a href="pedido-espuma-vertical.html" class="sub-card-cta">Personalizar</a>
      </div>
    </article>

    <article class="sub-card">
      <div class="sub-card-img">
        <span class="sub-card-price">Desde ${prices_apai[0]}</span>
        <img class="img-base" src="{IMG_HOVER}" alt="Panel de espuma apaisado">
        <img class="img-hover" src="{IMG_BASE}" alt="Panel de espuma apaisado en pared">
      </div>
      <div class="sub-card-body">
        <h2 class="sub-card-title">Panel de espuma apaisado</h2>
        <a href="pedido-espuma-apaisado.html" class="sub-card-cta">Personalizar</a>
      </div>
    </article>

    <article class="sub-card">
      <div class="sub-card-img">
        <span class="sub-card-price">Desde ${prices_cuad[0]}</span>
        <img class="img-base" src="{IMG_BASE}" alt="Panel de espuma cuadrado">
        <img class="img-hover" src="{IMG_HOVER}" alt="Panel de espuma cuadrado en pared">
      </div>
      <div class="sub-card-body">
        <h2 class="sub-card-title">Panel de espuma cuadrado</h2>
        <a href="pedido-espuma-cuadrado.html" class="sub-card-cta">Personalizar</a>
      </div>
    </article>
...
with open(os.path.join(OUT_DIR, "arte-mural-espuma.html"), "w", encoding="utf-8") as f:
    f.write(SUB_CATALOG)
print("OK arte-mural-espuma.html")


# ===== PEDIDO TEMPLATE =====
def build_pedido(slug, full_name, sizes, prices):
    options = []
    for i, (val, label) in enumerate(sizes):
        p = prices[i]
        options.append(f'        <option data-price="{p}" value="{val} · ${p}">{label}  —  ${p}</option>')
    options_html = '\n'.join(options)
...
<meta http-equiv="Expires" content="0">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Hacer mi pedido · {full_name} · Kodara Print</title>
<link rel="icon" type="image/jpeg" href="logo-kodara.jpeg">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&family=JetBrains+Mono:wght@400;600;700&display=swap" rel="stylesheet">
<style>
...
.color-opt{{cursor:pointer;background:var(--bg-2);border:2px solid var(--border-2);border-radius:14px;padding:20px;text-align:center;transition:all 0.2s;display:flex;flex-direction:column;align-items:center;gap:12px}}
.color-opt:hover{{border-color:var(--orange-light)}}
.color-opt input{{display:none}}
.color-opt:has(input:checked){{border-color:var(--orange);background:rgba(249,115,22,0.08)}}
.color-swatch{{width:56px;height:56px;border-radius:50%;border:2px solid rgba(255,255,255,0.15);box-shadow:0 4px 14px rgba(0,0,0,0.3)}}
.color-swatch.negro{{background:#0a0a0a}}
.color-swatch.blanco{{background:#fafafa;border-color:rgba(255,255,255,0.4)}}
...
<nav class="topnav">
  <a href="arte-mural-espuma.html" class="topnav-logo">
    <img src="logo-kodara.jpeg" alt="Kodara SE">
    <div>Kodara <span>PRINT</span></div>
  </a>
  <a href="arte-mural-espuma.html" class="topnav-back">← Volver a Impresión en espuma</a>
</nav>

<header class="page-header">
  <div class="page-header-inner">
    <div class="page-tag">Pedido · {full_name}</div>
    <h1 class="page-title">Haz tu <span class="grad">pedido</span>.</h1>
    <p class="page-sub">Elige color, tamaño, sube tu imagen y déjanos tus datos. Nosotros nos encargamos del resto.</p>
...
<form class="form-wrap" action="https://api.web3forms.com/submit" method="POST">
  ...
  <input type="hidden" name="subject" value="Nuevo pedido · {full_name} · Kodara Print">
  ...
  <div class="form-step">
    <div class="step-head">
      ...
      <div class="step-title">Color del panel</div>
    </div>
    <p class="step-sub">El canto (borde lateral) del panel será del color que elijas.</p>
    <div class="color-grid">
      <label class="color-opt">
        <input type="radio" name="color_panel" value="Blanco" required>
        <div class="color-swatch blanco"></div>
        <div class="color-label">Blanco</div>
      </label>
      <label class="color-opt">
        <input type="radio" name="color_panel" value="Negro">
        <div class="color-swatch negro"></div>
  ...
  <div class="form-step">
    <div class="step-head">
      ...
      <div class="step-title">Elige el tamaño</div>
    </div>
    <p class="step-sub">{len(sizes)} tamaños disponibles desde ${prices[0]} hasta ${prices[-1]}.</p>
...
{options_html}
  ...
  <div class="form-step">
    <div class="step-head">
      ...
      <div class="step-title">Tus datos de envío</div>
    </div>
...
form.addEventListener('submit', (e) => {{
  const color = form.querySelector('[name="color_panel"]:checked');
  if(!color){{e.preventDefault();showAlert('Tienes que elegir el color del panel (paso 1).');return;}}
  const tamano = form.querySelector('[name="tamano"]');
  if(!tamano.value){{e.preventDefault();showAlert('Tienes que elegir un tamaño (paso 2).');tamano.focus();return;}}
  if(!urlField.value || !urlField.value.startsWith('https://res.cloudinary.com/')){{
    e.preventDefault();showAlert('Tienes que subir una imagen (paso 3).');zone.scrollIntoView({{behavior:'smooth',block:'center'}});return;
  }}
...
with open(os.path.join(OUT_DIR, "pedido-espuma-vertical.html"), "w", encoding="utf-8") as f:
    f.write(build_pedido("espuma-vertical", "Panel vertical en espuma", SIZES_VERTICAL, prices_vert))
print("OK pedido-espuma-vertical.html")

with open(os.path.join(OUT_DIR, "pedido-espuma-apaisado.html"), "w", encoding="utf-8") as f:
    f.write(build_pedido("espuma-apaisado", "Panel de espuma apaisado", SIZES_APAISADO, prices_apai))
print("OK pedido-espuma-apaisado.html")

with open(os.path.join(OUT_DIR, "pedido-espuma-cuadrado.html"), "w", encoding="utf-8") as f:
    f.write(build_pedido("espuma-cuadrado", "Panel de espuma cuadrado", SIZES_CUADRADO, prices_cuad))
print("OK pedido-espuma-cuadrado.html")
