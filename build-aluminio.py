"""
Construye la categoria de Aluminio: sub-catalogo + 2 paginas de pedido.
- arte-mural-aluminio.html
- pedido-aluminio-cepillado.html
- pedido-aluminio.html
"""
import os
import sys
...
# 14 tamanos (los mismos que Lienzos)
SIZES = [
...
# Margen 50%: price = (cost + ship) * 2, redondeado a entero
# Aluminio Cepillado: cost $22.92 - $91.07
# Aluminio Standard: cost $14.32 - $57.31
# Envio: $10.56 - $31.83
def interp(a, b, pos, total):
    return a + (b - a) * pos / (total - 1)

def price(cost_min, cost_max, pos):
    cost = interp(cost_min, cost_max, pos, 14)
    ship = interp(10.56, 31.83, pos, 14)
    return round((cost + ship) * 2)

cepillado_prices = [price(22.92, 91.07, i) for i in range(14)]
standard_prices = [price(14.32, 57.31, i) for i in range(14)]

print("Cepillado:", cepillado_prices)
print("Standard: ", standard_prices)

# Imagenes (de las que ya tenemos en personalizar-arte-mural.html)
IMG_CEPILLADO_BASE = "https://d8j0ntlcm91z4.cloudfront.net/user_3DYM1J3h5iy8QkCLK3SvRJRjyvr/hf_20260523_131559_e7fde052-6657-4051-9e5f-884e4775a92b.png"
IMG_CEPILLADO_HOVER = "https://d8j0ntlcm91z4.cloudfront.net/user_3DYM1J3h5iy8QkCLK3SvRJRjyvr/hf_20260523_130830_45e2b9de-0879-4e0c-adb6-1075927b4821.png"
# Para Standard usamos las mismas pero invertidas (no tenemos imagenes especificas)
IMG_STANDARD_BASE = IMG_CEPILLADO_HOVER
IMG_STANDARD_HOVER = IMG_CEPILLADO_BASE

# ========== 1. ARTE-MURAL-ALUMINIO.HTML ==========
...
<title>Impresión en aluminio · Kodara Print Studio</title>
...
<header class="page-header">
  <div class="page-header-inner">
    <div class="page-tag">Arte Mural · Impresión en aluminio</div>
    <h1 class="page-title">
      Personaliza tu <span class="grad">impresión en aluminio</span>.
    </h1>
    <p class="page-sub">
      Impresiones directas sobre aluminio. Acabado moderno, resistente y duradero. Listas para colgar.
    </p>
  </div>
</header>
  ...
  <div class="sub-services">
    ...
    <article class="sub-card">
      <div class="sub-card-img">
        <span class="sub-card-price">Desde ${cepillado_prices[0]}</span>
        <img class="img-base" src="{IMG_CEPILLADO_BASE}" alt="Impresión en aluminio cepillado">
        <img class="img-hover" src="{IMG_CEPILLADO_HOVER}" alt="Impresión en aluminio cepillado en pared">
      </div>
      <div class="sub-card-body">
        <h2 class="sub-card-title">Impresión en aluminio cepillado</h2>
        <a href="pedido-aluminio-cepillado.html" class="sub-card-cta">Personalizar</a>
      </div>
    </article>

    <article class="sub-card">
      <div class="sub-card-img">
        <span class="sub-card-price">Desde ${standard_prices[0]}</span>
        <img class="img-base" src="{IMG_STANDARD_BASE}" alt="Impresión en aluminio">
        <img class="img-hover" src="{IMG_STANDARD_HOVER}" alt="Impresión en aluminio en pared">
      </div>
      <div class="sub-card-body">
        <h2 class="sub-card-title">Impresión en aluminio</h2>
        <a href="pedido-aluminio.html" class="sub-card-cta">Personalizar</a>
      </div>
    </article>
...
with open(os.path.join(OUT_DIR, "arte-mural-aluminio.html"), "w", encoding="utf-8") as f:
    f.write(SUB_CATALOG)
print("OK arte-mural-aluminio.html")


# ========== TEMPLATE PEDIDO ==========
def build_pedido(slug, full_name, prices, min_price, max_price):
    # Construir opciones del select agrupadas
    groups = {"pequenos": [], "medianos": [], "grandes": []}
    for i, (val, label, group) in enumerate(SIZES):
        price = prices[i]
        groups[group].append(
            f'        <option data-price="{price}" value="{val} · ${price}">{label}  —  ${price}</option>'
        )

    options_html = f'''      <optgroup label="Tamaños pequeños · ${prices[0]}–${prices[3]}">
{chr(10).join(groups["pequenos"])}
      </optgroup>

      <optgroup label="Tamaños medianos · ${prices[4]}–${prices[8]}">
{chr(10).join(groups["medianos"])}
      </optgroup>

      <optgroup label="Tamaños grandes · ${prices[9]}–${prices[13]}">
...
<nav class="topnav">
  <a href="arte-mural-aluminio.html" class="topnav-logo">
    <img src="logo-kodara.jpeg" alt="Kodara SE">
    <div>Kodara <span>PRINT</span></div>
  </a>
  <a href="arte-mural-aluminio.html" class="topnav-back">← Volver a Impresión en aluminio</a>
</nav>
...
<header class="page-header">
  <div class="page-header-inner">
    ...
    <p class="page-sub">Elige tamaño, orientación, sube tu imagen y déjanos tus datos. Nosotros nos encargamos del resto.</p>
...
<form class="form-wrap" action="https://api.web3forms.com/submit" method="POST">
  ...
  <div class="form-step">
    ...
    <p class="step-sub">14 tamaños disponibles desde ${min_price} hasta ${max_price}.</p>
...
{options_html}
  ...
  <div class="form-step">
    <div class="step-head">
      ...
      <div class="step-title">Orientación</div>
    </div>
...
form.addEventListener('submit', (e) => {{
  ...
  if(!tamano.value){{e.preventDefault();showAlert('Tienes que elegir un tamaño (paso 1).');tamano.focus();return;}}
  const orient = form.querySelector('[name="orientacion"]:checked');
  if(!orient){{e.preventDefault();showAlert('Tienes que elegir orientación (paso 2).');return;}}
...
# Generar las 2 paginas pedido
with open(os.path.join(OUT_DIR, "pedido-aluminio-cepillado.html"), "w", encoding="utf-8") as f:
    f.write(build_pedido("aluminio-cepillado", "Impresión en aluminio cepillado", cepillado_prices, cepillado_prices[0], cepillado_prices[-1]))
print("OK pedido-aluminio-cepillado.html")

with open(os.path.join(OUT_DIR, "pedido-aluminio.html"), "w", encoding="utf-8") as f:
    f.write(build_pedido("aluminio", "Impresión en aluminio", standard_prices, standard_prices[0], standard_prices[-1]))
print("OK pedido-aluminio.html")
