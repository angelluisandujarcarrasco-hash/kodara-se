"""
Aplica precios CON ENVÍO INCLUIDO (margen 50%) a los 5 formularios de Marco de Madera.
Costos: producto + envío $12.99-$36.99
También actualiza los badges 'Desde $X' del catálogo arte-mural-marco-madera.html
"""
import os
import re
...
# 5 productos con sus costos min/max
products = [
    {
        "key": "mate-premium",
        "card_text": "Mate Prémium con Marco de Madera",
        "costo_min": 22.03, "costo_max": 105.56,
    },
    {
        "key": "mate-clasico",
        "card_text": "Mate Clásico con Marco de Madera",
        "costo_min": 20.17, "costo_max": 100.04,
    },
    {
        "key": "semibrillante-clasico",
        "card_text": "Semibrillante Clásico con Marco de Madera",
        "costo_min": 19.55, "costo_max": 99.02,
    },
    {
        "key": "semibrillante-premium",
        "card_text": "Semibrillante Prémium con Marco de Madera",
        "costo_min": 21.61, "costo_max": 104.58,
    },
    {
        "key": "mate-museo",
        "card_text": "Mate Calidad Museo con Marco de Madera",
        "costo_min": 26.74, "costo_max": 116.41,
    },
]
...
# 29 posiciones relativas (curva similar a Pósteres - no lineal, refleja realidad de Gelato)
# Estas posiciones van de 0 a 1
posiciones = [
...
def calc_prices(costo_min, costo_max):
    """Calcula los 29 precios con margen 50% sobre (costo producto + envio)"""
    prices = []
...
# Cargar catálogo para actualizar badges al final
catalogo_path = os.path.join(OUT_DIR, "arte-mural-marco-madera.html")
with open(catalogo_path, "r", encoding="utf-8") as f:
    cat = f.read()

for prod in products:
    prices = calc_prices(prod["costo_min"], prod["costo_max"])
    filename = f"pedido-marco-madera-{prod['key']}.html"
    ...
    new_stepsub = f'<p class="step-sub">El tamaño final del póster con marco que recibirás. Disponibles 29 tamaños desde ${pmin} hasta ${pmax}.</p>'
    ...
    # Verificar si la tarjeta ya tiene sub-card-price
    card_text = prod["card_text"]
    # Patrón para encontrar la tarjeta de este producto
    # Busca: <article class="sub-card"> ... <h2 class="sub-card-title">CARD_TEXT</h2>
    # Si NO tiene sub-card-price, hay que añadirlo
    article_pattern = re.compile(
        r'(<article class="sub-card">\s*<div class="sub-card-img">\s*)(<img[^>]*>\s*<img[^>]*>\s*</div>\s*<div class="sub-card-body">\s*<h2 class="sub-card-title">' + re.escape(card_text) + r'</h2>)',
        re.DOTALL
    )
    new_badge = f'<span class="sub-card-price">Desde ${pmin}</span>\n        '
    if 'sub-card-price' not in cat[max(0, cat.find(card_text)-500):cat.find(card_text)]:
        # Insertar antes de <img class="img-base"
        cat, n = article_pattern.subn(r'\1' + new_badge + r'\2', cat)
        if n > 0:
            print(f"  Badge añadido para {card_text}: Desde ${pmin}")
        else:
            print(f"  ⚠ NO MATCH badge para {card_text}")

# Asegurar que el CSS de .sub-card-price existe en arte-mural-marco-madera.html
if '.sub-card-price' not in cat:
    ...
    print("  CSS .sub-card-price añadido al catálogo")

with open(catalogo_path, "w", encoding="utf-8") as f:
    f.write(cat)
print("OK arte-mural-marco-madera.html (badges actualizados)")

print("\nDONE — 5 paginas de pedido + catálogo Marco de Madera actualizados con envío incluido")
