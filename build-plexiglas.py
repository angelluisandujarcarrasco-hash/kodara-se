"""
Construye pedido-plexiglas.html (1 sola sub-categoria).
"""
import os
import sys
...
def price(cost_min, cost_max, ship_min, ship_max, pos):
    cost = interp(cost_min, cost_max, pos, 14)
    ship = interp(ship_min, ship_max, pos, 14)
    return round((cost + ship) * 2)

# Plexiglas: cost $20.94 - $87.08, envio $10.78 - $32.52
prices = [price(20.94, 87.08, 10.78, 32.52, i) for i in range(14)]
print("Plexiglas:", prices)


def build_pedido(full_name, prices, back_url, back_label):
    groups = {"pequenos": [], "medianos": [], "grandes": []}
    for i, (val, label, group) in enumerate(SIZES):
        p = prices[i]
        groups[group].append(
            f'        <option data-price="{p}" value="{val} · ${p}">{label}  —  ${p}</option>'
        )
...
<nav class="topnav">
  <a href="{back_url}" class="topnav-logo">
    <img src="logo-kodara.jpeg" alt="Kodara SE">
    <div>Kodara <span>PRINT</span></div>
  </a>
  <a href="{back_url}" class="topnav-back">← Volver a {back_label}</a>
</nav>
...
<form class="form-wrap" action="https://api.web3forms.com/submit" method="POST">
  ...
  <div class="form-step">
    ...
    <p class="step-sub">14 tamaños disponibles desde ${prices[0]} hasta ${prices[-1]}.</p>
...
with open(os.path.join(OUT_DIR, "pedido-plexiglas.html"), "w", encoding="utf-8") as f:
    f.write(build_pedido("Impresión en plexiglás", prices, "personalizar-arte-mural.html", "Arte Mural"))
print("OK pedido-plexiglas.html")
print("\nDONE")
